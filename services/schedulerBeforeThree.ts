...
export const schedulerBeforeThree = () => {
  const queryBeforeThree = `SELECT u.username, c.clientPassword, c.clientName, c.clientEmail, c.application, c.mac, c.keyApplication, c.whatsappNumber, DATE_FORMAT(c.startDate, '%Y-%m-%d') as expired_date, tm.message, p.productName, p.price as product_price, pl.planName, pl.price as plan_price, CASE WHEN c.invoiceStatus = 0 THEN 'Pending' WHEN c.invoiceStatus = 1 THEN 'Paid' ELSE 'Unknown' END AS invoiceStatus FROM clients c JOIN templateMessages tm ON c.userId = tm.userId JOIN plans pl ON c.plan = pl.id JOIN products p ON c.product = p.id JOIN users u ON c.userId = u.id WHERE c.reminderBeforeThree = 1 AND tm.codeMessage = 3 AND CURDATE() >= DATE_SUB(c.startDate, INTERVAL 3 DAY) AND CURDATE() < DATE_SUB(c.startDate, INTERVAL 2 DAY);`

  executeQuery(queryBeforeThree)
    .then((results) => {
