  ...
  if (req.method === "POST") {
    ...
    checkAuthorizationHeader(
      async (req: NextApiRequest, res: NextApiResponse) => {
        ...
        // const queryGetClientData = `select whatsappNumber from clients WHERE userId = '${userId}' AND product = '${productId}'`;

        const queryAfterOne = `SELECT u.username, c.clientPassword, c.clientName, c.clientEmail, c.application, c.mac, c.keyApplication, c.whatsappNumber, DATE_FORMAT(c.startDate, '%Y-%m-%d') as expired_date, p.productName, p.price as product_price, pl.planName, pl.price as plan_price FROM clients c JOIN plans pl ON c.plan = pl.id JOIN products p ON c.product = p.id JOIN users u ON c.userId = u.id WHERE c.userId = '${userId}' AND product = '${productId}'`;
